...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Homework9) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(define resolve
  (lambda (varName env)
    (cond
      ((null? env) #f)
...
;check if param exists in env, if it does, we want that param to be local.
...
parsed-no-code (no-parser sample-no-code)
