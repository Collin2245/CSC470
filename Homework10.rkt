...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Homework10) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
  ...
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'let-exp)
       (let* ((var-2-lists (cadr parsed-no-code))
              (new-env (extend-env var-2-lists env))
...
(define sample-no-code '(call (function () (local-vars ((a 4) (b 11)) (do-mathy-stuff + b a)))))
