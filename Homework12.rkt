...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Homework12) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
...
(define clean-block-exp
  (lambda (blockOutput)
    (cond
      ((null? blockOutput) '())
      ((eq? (car blockOutput) 0)
       (clean-block-exp (cdr blockOutput)))
      (else
       (cons (car blockOutput) (clean-block-exp (cdr blockOutput)))))))
  ...
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car parsed-no-code) 'spit-out-exp)
       (list 'Output (run-parsed-code (cadr parsed-no-code) env)))
...
(define sample-define-code '(block (remember a 2) (remember b 11) (spit-out a) (spit-out b) (remember c (do-mathy-stuff + a b)) (remember b 0) (spit-out c) (spit-out b)))
